// hackathon_may_stackup\moodify\src\app\components\CloudinaryMusicPlayer.tsx
...
interface CloudinaryMusicPlayerProps {
  track: CloudinaryTrack;
  onClose: () => void;
  autoPlay?: boolean;
  onPrevious?: () => void;
  onNext?: () => void;
...
const CloudinaryMusicPlayer = forwardRef(
  (
    {
      track,
      onClose,
    ...
    const [localUserInteracted, setLocalUserInteracted] =
      useState(userInteracted);
      ...
      playTrack: () => {
        if (audioRef.current && isAudioReady) {
          ...
          context.resume().then(() => {
            if (audioRef.current) {
              console.log(
                "Attempting to play Cloudinary track via playTrack method"
              );
              ...
              if (playPromise !== undefined) {
                ...
                playPromise
                  .then(() => {
                    console.log(
                      "Cloudinary track started playing successfully"
                    );
                  })
                  .catch((err) => {
    ...
    useEffect(() => {
      ...
      console.log("Loading Cloudinary audio from:", track.cloudinaryUrl);
      ...
      const handleCanPlay = () => {
        console.log("Cloudinary audio can play now");
        setIsAudioReady(true);

        if (autoPlay && localUserInteracted) {
          console.log("Attempting auto-play for Cloudinary track");
          const promise = audio
            .play()
            .then(() => {
      ...
      const handleLoadedMetadata = () => {
        console.log(
          "Cloudinary audio metadata loaded, duration:",
          audio.duration
        );
      ...
      const handleError = () => {
        console.error("Cloudinary audio error:", audio.error);
        ...
        if (loadAttempts < 3) {
          ...
          setTimeout(() => {
            if (audioRef.current) {
              audioRef.current.src = track.cloudinaryUrl;
              audioRef.current.load();
            }
          }, 1000);
        } else {
          setAudioError(
            "Failed to load audio. Please try again or try another track."
          );
        }
      };
    ...
    useEffect(() => {
      const handleUserInteraction = () => {
        ...
        if (autoPlay && isAudioReady && !isPlaying && audioRef.current) {
          const promise = audioRef.current
            .play()
    ...
    useEffect(() => {
      ...
      if (isPlaying) {
        console.log("Attempting to play Cloudinary audio");
          ...
          playPromise
            ...
            .catch((err) => {
              ...
              if (err.name === "NotAllowedError") {
                ...
                setAudioError(
                  "Playback failed. Try again or try another track."
                );
              }
            });
        }
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <img
                src={track.coverImage || "/placeholder-album.png"}
                alt={track.title}
...
CloudinaryMusicPlayer.displayName = "CloudinaryMusicPlayer";

export default CloudinaryMusicPlayer;
