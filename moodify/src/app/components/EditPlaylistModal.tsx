// hackathon_may_stackup\moodify\src\app\components\EditPlaylistModal.tsx
...
interface EditPlaylistModalProps {
  playlist: (CreatedPlaylist & { _id?: string });
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}
...
export default function EditPlaylistModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/playlists/update?id=${playlist._id}`, {
        method: "PUT",
        body: formData,
      });
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex space-x-3">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="flex-1 bg-purple-600 hover:bg-purple-700 text-white font-medium py-2 px-4 rounded-md transition-colors disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center"
              >
                {isLoading ? (
                  <>
